...
import FaceTrackingComponent from './faceTrackingComponent'; // Import the FaceTrackingComponent
...
export default function Navbar({ serviceRef = null, blogRef = null }) {
  ...
  const [isFaceTrackingEnabled, setIsFaceTrackingEnabled] = useState(() => {
    // Read from localStorage or default to false
    return JSON.parse(localStorage.getItem('faceTrackingEnabled')) || false;
  });
  ...
  const toggleFaceTracking = () => {
    setIsFaceTrackingEnabled(prevState => {
      const newState = !prevState;
      localStorage.setItem('faceTrackingEnabled', JSON.stringify(newState)); // Save state to localStorage
      // console.log('Face tracking toggled to:', newState); // Add log
      return newState;
    });
  };
  
  // console.log('Rendering Navbar with isFaceTrackingEnabled:', isFaceTrackingEnabled);
  ...
  return (
    <>
          ...
          <div className="collapse navbar-collapse" id="navbarNav">
            ...
            <ul className="navbar-nav ms-3">
              ...
              <li className="nav-item">
                ...
                <Modal show={isModalVisible} onHide={handleCloseModal} className="custom-modal" aria-labelledby="modal-title" aria-hidden="true">
                  ...
                  <Modal.Body>
                    {isLoggedIn ? (
                      ...
                      <div>
                        <Button variant="primary" onClick={() => navigate('/signin', { state: { from: location.pathname } })}>Sign In</Button>
                        <Button variant="secondary" onClick={() => navigate('/signup', { state: { from: location.pathname } })}>Sign Up</Button>
                      </div>
                    )}
                  </Modal.Body>
                </Modal>
      ...
      {/* Only include FaceTrackingComponent if the toggle state changes */}
      {/* <FaceTrackingComponent isFaceTrackingEnabled={isFaceTrackingEnabled} /> */}
    </>
  );
}
